...
export default function Signin() {
	...
	const handleSignin = useCallback(async () => {
		...
		if (data.user) {
			try {
				// First, save user profile data
				await fetch("/api/auth/user", {
					method: "POST",
					headers: { "Content-Type": "application/json" },
					body: JSON.stringify({ user: data.user }),
				});
				// Then, log the session event
				await fetch("/api/auth/log-session", {
					method: "POST",
					headers: { "Content-Type": "application/json" },
					body: JSON.stringify({ user: data.user, event_type: "login" }),
				});
			} catch (apiError) {
	...
	const handleGoogleSignin = useCallback(async () => {
		...
		try {
			...
			// Supabase will handle redirect to callback; UI state remains pending.
		} catch (e) {
			console.error(e);
